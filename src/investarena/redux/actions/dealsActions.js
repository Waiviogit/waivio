...
export function createOpenDealApi (dealData) {
    return (dispatch) => {
        return api.deals.createOpenDeal(dealData)
            .then(({ error }) => {
                if (!error) {
                    if (dealData.post_id) {
...
export function getPostOpenDeals (postId) {
    return (dispatch) => {
        return api.deals.getPostOpenDeals({post_id: postId})
            .then(({ data, error }) => {
                if (data && !error) {
...
export function getLastClosedDealForStatistics (platformName) {
    return () => {
        return api.deals.getLastClosedDeal(platformName)
            .then(({ data, error }) => {
                if (data && !error) {
                    if (_.isEmpty(data.last_closed_deal)) {
                        ...
                        if (diff <= 1) {
                            ...
                            singleton.platform.getClosedDeals('LAST_7_DAYS', true, data.last_closed_deal.close_time);
                        } else if (diff > 7 && diff <= 30) {
                            singleton.platform.getClosedDeals('LAST_30_DAYS', true, data.last_closed_deal.close_time);
                        } else if (diff > 30 && diff <= 60) {
                            singleton.platform.getClosedDeals('LAST_60_DAYS', true, data.last_closed_deal.close_time);
                        } else if (diff > 60 && diff <= 90) {
                            singleton.platform.getClosedDeals('LAST_90_DAYS', true, data.last_closed_deal.close_time);
                        } else if (diff > 90 && diff <= 180) {
                            singleton.platform.getClosedDeals('LAST_6_MONTHS', true, data.last_closed_deal.close_time);
                        } else {
                            singleton.platform.getClosedDeals('LAST_6_MONTHS', true, data.last_closed_deal.close_time);
                        }
                    }
