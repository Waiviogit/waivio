...
class PostDifference extends Component {
    ...
    profit = (quoteSettings, quote) => {
        if (quote.askPrice !== '-' && quote.bidPrice !== '-') {
            if (this.props.recommend.toLowerCase() === 'buy') {
                return Math.trunc((quote.askPrice * 1000000 - this.props.postPrice * 1000000) / quoteSettings.tickSize);
            } else {
                return Math.trunc((this.props.postPrice * 1000000 - quote.bidPrice * 1000000) / quoteSettings.tickSize);
            }
        } else {
            return '-';
        }
    ...
    render () {
        return (
            <div className={this.state.isProfitGray
                ? 'st-post-profit-gray'
