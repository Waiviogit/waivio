...
class ModalTakeProfit extends Component {
  ...
  render() {
    let buttonText = this.props.intl.formatMessage({
      id: 'deals.takeProfit',
      defaultMessage: 'Take profit',
    });
    if (this.props.openDeal.takeProfitPrice) {
      buttonText = quoteFormat(
        this.props.openDeal.takeProfitPrice / 1000000,
    ...
    return (
      <div className="st-open-deal-button-wrap">
        <button
          id="modalTakeProfitOpen"
          className="btn st-open-deal-button-blue"
          onClick={this.toggle}
        >
          {this.props.openDeal.takeProfitAmount || this.props.openDeal.takeProfitPrice ? (
            <span>TP: </span>
          ) : null}
          {buttonText}
        </button>
