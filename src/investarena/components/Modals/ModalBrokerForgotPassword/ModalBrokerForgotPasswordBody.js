...
class ModalBrokerForgotPasswordBody extends Component {
    ...
    componentDidMount () {
        ...
        if (this.emailInput) {
            validateRegexWithTooltip(this.emailInput, 'change', validateRegistrationSignIn[this.emailInput.getAttribute('data-validate')]);
        }
    }
    handleClick = () => {
        let isValid = true;
        if (this.emailInput && this.emailInput.value === '') {
            ...
            this.emailInput.parentElement.setAttribute('data-tooltip', this.emailInput.getAttribute('data-empty'));
            isValid = false;
            ...
            <div>
                ...
                <div className="st-modal-broker-forgot-password-input-wrap" data-position="top">
                    <input
                        ...
                        maxLength={256}
                        type="email"
                        placeholder={this.props.intl.formatMessage({ id: 'modalBrokerForgotPassword.modalBody.emailPlaceholder' })}
                        data-validate="email"
                        data-title={this.props.intl.formatMessage({ id: 'tooltip.emailValid' })}
                        data-empty={this.props.intl.formatMessage({ id: 'tooltip.empty' })}/>
                </div>
                <button disabled={this.props.isLoading} onClick={this.handleClick} className="btn btn-primary st-modal-broker-forgot-password-button">
