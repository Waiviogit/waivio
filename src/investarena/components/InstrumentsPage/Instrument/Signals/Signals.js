...
const Signals = ({ intl, signals }) =>
  signals && signals.length > 0 ? (
    <Popover
      ...
      content={_.map(formatSignalsData(signals), signal => (
        <div className="st-signal-wrap" key={`signal:${signal.id}`}>
          ...
          <div
            className={`st-signal-recommendation st-signal-color-${
              signal.recommendation === 'buy' ? 'success' : 'danger'
            }`}
          >
            <FormattedMessage
              id={`signal.${signal.recommendation}`}
              defaultMessage={signal.recommendation === 'buy' ? 'Buy' : 'Sell'}
            />
          </div>
    ...
    >
      <div className="st-signals-button">{signals.length}</div>
    </Popover>
  ) : null;
