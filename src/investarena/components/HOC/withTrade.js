    ...
    class WithTrade extends React.Component {
        ...
        componentDidMount () {
            if (this.props.quote && this.props.quoteSettings) {
                ...
                const amount = this.props.amountModal || numberFormat(amountValue, PlatformHelper.countDecimals(amountValue));
        ...
        componentWillReceiveProps (nexProps) {
            if (nexProps.quote && nexProps.quoteSettings) {
                if ((this.state.amount === '' || this.state.margin === '' || this.state.margin === '---' || (!this.props.quote || !this.props.quoteSettings))) {
                    const amountValue = nexProps.quoteSettings.defaultQuantity / 1000000;
                    const amount = this.props.amountModal || numberFormat(amountValue, PlatformHelper.countDecimals(amountValue));
                    const margin = this.props.marginModal || PlatformHelper.getMargin(nexProps.quote, nexProps.quoteSettings, amount);
                    this.setState({ amount, margin });
                } else {
                    const margin = PlatformHelper.getMargin(nexProps.quote, nexProps.quoteSettings, this.state.amount);
        ...
        handleChangeInput = (e) => {
            ...
            const margin = PlatformHelper.getMargin(this.props.quote, this.props.quoteSettings, amount);
            e.persist();
            this.setState({amount, margin}, () => { e.target.selectionStart = e.target.selectionEnd = position });
        };
    ...
    function mergeProps (stateProps, dispatchProps, ownProps) {
        ...
        return {
            ...
            createOpenDeal: (side, amount, margin) => {
                if (platformName !== 'widgets' && !isOpen && !isOneClickTrade && isSignIn && quote && quoteSettings && (side === 'Sell' || side === 'Buy')) {
                    dispatch(toggleModal('openDeals', {quote, quoteSettings, side, amount, margin, postId, platformName}));
                } else if (!isSignIn) {
                    dispatch(toggleModal('authorizeSinglePost'));
                } else if (platformName === 'widgets' && isSignIn) {
                    dispatch(toggleModal('broker'));
                } else if (platformName !== 'widgets' && !isOpen && isSignIn && isOneClickTrade && quote && quoteSettings && (side === 'Sell' || side === 'Buy')) {
                    dispatch(createOpenDealPlatform(quote, quoteSettings, side, amount, margin, postId, platformName));
                } else if (platformName !== 'widgets' && isOpen && isSignIn && quote && quoteSettings && (side === 'Sell' || side === 'Buy')) {
                    dispatch(createOpenDealPlatform(quote, quoteSettings, side, amount, margin, postId, platformName));
                }
            }
        };
