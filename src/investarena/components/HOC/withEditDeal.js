...
const withEditDeal = Component => {
  class WithEditDeal extends React.Component {
    ...
    componentDidMount() {
      ...
      const ranges = PlatformHelper.getRanges(
        this.props.quote,
        this.props.openDeal,
    ...
    handleChangeInputSL = () => {
      if (
        PlatformHelper.validateChangeProfitLossText(
          this.inputSL,
          this.state.ranges.stopLoss[this.state.chosenRangesSL].min,
          this.state.ranges.stopLoss[this.state.chosenRangesSL].max,
        )
      ) {
    ...
    handleChangeInputTP = () => {
      if (
        PlatformHelper.validateChangeProfitLossText(
          this.inputTP,
          this.state.ranges.takeProfit[this.state.chosenRangesTP].min,
          this.state.ranges.takeProfit[this.state.chosenRangesTP].max,
        )
      ) {
    ...
    handleClickSetSL = () => {
      if (this.inputSL.value.length === 0) {
        if (this.props.openDeal.stopLossPrice || this.props.openDeal.stopLossAmount) {
          changeOpenDealPlatform(
            this.props.openDeal.dealId,
            null,
            null,
            this.props.openDeal.takeProfitPrice,
            this.props.openDeal.takeProfitAmount,
          );
          this.props.toggle();
        } else {
          this.inputSL.classList.add('st-input-danger');
        ...
        message.error(
          this.props.intl.formatMessage({
            id: 'deals.stopLossWrongValue',
            defaultMessage: 'Please enter a value in the acceptable range',
          }),
        );
      }
    };
    handleClickSetTP = () => {
      if (this.inputTP.value.length === 0) {
        if (this.props.openDeal.takeProfitPrice || this.props.openDeal.takeProfitAmount) {
          changeOpenDealPlatform(
            this.props.openDeal.dealId,
            this.props.openDeal.stopLossPrice,
            this.props.openDeal.stopLossAmount,
            null,
            null,
          );
          this.props.toggle();
    ...
    handleClickSetSLTP = () => {
      ...
      let isValidSL = PlatformHelper.validateChangeProfitLossText(
        this.inputSL,
        this.state.ranges.stopLoss[this.state.chosenRangesSL].min,
        this.state.ranges.stopLoss[this.state.chosenRangesSL].max,
      );
      let isValidTP = PlatformHelper.validateChangeProfitLossText(
        this.inputTP,
        this.state.ranges.takeProfit[this.state.chosenRangesTP].min,
        this.state.ranges.takeProfit[this.state.chosenRangesTP].max,
      );
      if (this.state.isAmountTP) {
        if (this.inputTP.value.length === 0 && openDeal.takeProfitAmount) {
          ...
          this.inputTP.value.length === 0 &&
          !openDeal.takeProfitAmount &&
          !(openDeal.stopLossAmount || openDeal.stopLossPrice)
        ) {
          takeProfitAmount = null;
          isValidTP = false;
        } else {
      ...
      if (this.state.isAmountSL) {
        if (this.inputSL.value.length === 0 && openDeal.stopLossAmount) {
          ...
          this.inputSL.value.length === 0 &&
          !openDeal.stopLossAmount &&
          !(openDeal.takeProfitAmount || openDeal.takeProfitPrice)
        ) {
          stopLossAmount = null;
          isValidSL = false;
        } else {
