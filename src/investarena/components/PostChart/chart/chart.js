...
class Chart {
  ...
  drawLineChart(shortBars, quote, isExpired, expiredByTime) {
    ...
    shortBars.forEach((bar, i) => {
      const continueDrawing = isExpired ? expiredByTime || !bar.correctedBars : true;
      if (continueDrawing) {
  ...
  drawStopPriceLine(scaleY, stopPrices, recommend, brokenStopPrices) {
    const finishX =
      this.finishLineX > this.canvasWork.width || brokenStopPrices
  ...
  drawStartPolygon(recommend, brokenStopPrices) {
    ...
    const arrayDot = [
      [start + 10, start, start - 40, start - 40, start, start + 10],
      [
        polygonHeightCoord,
    ...
    const grd = this.ctx.createLinearGradient(
      startLine,
      polygonHeightCoord,
      startLine + (finishLine - startLine) + 25,
      polygonHeightCoord,
    );
    if (recommend === 'Buy') {
      grd.addColorStop(0.0, 'rgba(30, 190, 165, 1.000)');
      grd.addColorStop(1.0, 'rgba(255, 255, 255, 1.000)');
    } else if (recommend === 'Sell') {
      grd.addColorStop(0.0, 'rgba(238, 84, 81, 1.000)');
      grd.addColorStop(1.0, 'rgba(255, 255, 255, 1.000)');
    }
    ...
    this.drawText(
      start - 33,
      polygonHeightCoord + 4,
      recommend.toUpperCase(),
      '14px OpenSans, sans-serif',
      this.colors.polygon[recommend],
    );
    this.ctx.closePath();
  ...
  drawCandle(centerBar, currentBar, candleXCoord, askBid) {
    const candleYHighCoord =
      Math.round(this.getYByPrice(currentBar[askBid === 'closeAsk' ? 'highAsk' : 'highBid'])) + 0.5;
    const candleYLowCoord =
      Math.round(this.getYByPrice(currentBar[askBid === 'closeAsk' ? 'lowAsk' : 'lowBid'])) + 0.5;
    const candleYOpenCoord =
      Math.round(this.getYByPrice(currentBar[askBid === 'closeAsk' ? 'openAsk' : 'openBid'])) + 0.5;
    const candleYCloseCoordOffset = Math.round(
      (currentBar[askBid === 'closeAsk' ? 'openAsk' : 'openBid'] -
        currentBar[askBid === 'closeAsk' ? 'closeAsk' : 'closeBid']) *
        this.scaleY,
    );
    this.ctx.beginPath();
  ...
  drawTimeMark(shortBars, times) {
    ...
    for (let i = 1; i <= Math.floor(this.canvasWork.width / this.pxPerBar); i += 1) {
      if (!(this.pxPerGrid < 40 && i % 2 === 0)) {
        ...
        if (this.minutePerBar < MINUTE_PER_DAY) {
          // "Day": 60*24
          if (
            oldDate.toDate().getDay() !== date.toDate().getDay() &&
            Math.abs(startDate - date) > this.msecondsPerBar
          ) {
            this.drawText(
              i * this.pxPerGrid - 21,
          ...
          oldDate.toDate().getYear() !== date.toDate().getYear() &&
          Math.abs(startDate - date) > this.msecondsPerBar
        ) {
          this.drawText(
            i * this.pxPerGrid - 18,
