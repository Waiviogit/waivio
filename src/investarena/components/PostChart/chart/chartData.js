...
class ChartData {
    constructor ({
        createdAt, forecast, quoteSecurity, locale, tpPrice, slPrice, canvas, recommend
    }) {
        this.canvasWork = {
    ...
    updateData ({
        platform, data, quote, quoteSettings, chartType, priceType, isExpired, expiredByTime, timeScale, expiredAt, isScaleChanged
    }) {
        ...
        if (barsData && barsData.length > 1) {
            ...
            this.chartInfo = CanvasHelper.getChartInfo(this.times.created, this.times.forecast, this.updatedAt, this.canvasWork.width, isExpired, this.timeScale);
    ...
    getQuoteOptions (quote, priceType) {
        ...
        return {
            price, state, askBid, beforeStart
        };
    }
    ...
    correctBars = (bars) => {
        ...
        for (let i = bars.length - 1; i > 0; i -= 1) {
            ...
            for (let k = 1; k < gaps; k += 1) {
                correctedShortBars.push(!bar.quote
