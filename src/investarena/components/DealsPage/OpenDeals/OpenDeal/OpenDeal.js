...
const OpenDeal = ({ intl, quote, quoteSettings, openDeal, dealPnL, viewMode }) => {
    ...
    <div className="st-price">
      <div className="d-flex align-items-center justify-content-between">
        ...
        <div title={intl.formatMessage({ id: 'deals.opening', defaultMessage: 'opening' })}>
          {quoteFormat(openDeal.openPrice, quoteSettingsDeal)}
        </div>
      </div>
      <div className="d-flex align-items-center justify-content-between">
        <div className="st-open-deal-header-item-title">
        ...
        <div title={intl.formatMessage({ id: 'deals.market', defaultMessage: 'market' })}>
          {quoteFormat(marketPrice, quoteSettingsDeal)}
        </div>
      </div>
    </div>
  );
  ...
  const modalTP = (
    <ModalTakeProfit openDeal={openDeal} quote={quoteDeal} quoteSettings={quoteSettingsDeal} />
  );
  const modalSL = (
    <ModalStopLoss openDeal={openDeal} quote={quoteDeal} quoteSettings={quoteSettingsDeal} />
  );
  const modalCloseDeal = (
    <ModalClose openDealId={openDeal.dealId} openDealSequenceNumber={openDeal.dealSequenceNumber} />
  );
  switch (viewMode) {
    case 'cards':
      return (
