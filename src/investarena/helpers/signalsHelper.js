...
export const formatSignalsData = (signals) => {
    return _.orderBy(_.map(signals, signal => {
        return {
...
export const getCorrectTime = (interval, intl) => {
    ...
    shortTime += timeObject.days !== 0 ? timeObject.days + intl.formatMessage({id: 'signal.days'}) : '';
    shortTime += timeObject.hours !== 0 ? timeObject.hours + intl.formatMessage({id: 'signal.hours'}) : '';
    shortTime += timeObject.minutes !== 0 ? timeObject.minutes + intl.formatMessage({id: 'signal.minutes'}) : '';
    shortTime += timeObject.seconds !== 0 ? timeObject.seconds + intl.formatMessage({id: 'signal.seconds'}) : '';
    return shortTime;
};
