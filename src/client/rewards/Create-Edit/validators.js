...
export const validatorMessagesCreator = (messageFactory) => ({
  ...
  budgetToUSBDbalance: messageFactory('budget_overprices_wallet_balance', 'Budget should not exceed your SBD wallet balance'),
  rewardToZero: messageFactory('reward_more_than_zero', 'Reward should be more than zero'),
  rewardToBudget: messageFactory('reward_not_exceed_budget', 'The reward should not exceed the budget'),
  reservationPeriod: messageFactory('reserve_period_one_day', 'The reservation period must be at least one day'),
  photosQuality: messageFactory('not_less_zero_photos', 'Should not be less than zero photos'),
  commission: messageFactory('not_less_five_commission_value', 'Commissions must not be less than 5%'),
  minExpertise: messageFactory('reputation_cannot_be_negative', 'The Waivio reputation cannot be negative'),
  steemReputation: messageFactory('steem_reputation_from_100_to_100', 'The Steem reputation must be from -100 to 100'),
  followersQuality: messageFactory('not_less_zero_followers', 'Number of followers cannot be negative'),
  expiredDate: messageFactory('expiry_date_after_current', 'The expiry date must be after the current date'),
  postsQuantity: messageFactory('not_less_zero_posts', 'Number of posts cannot be negative'),
  nameField: messageFactory('not_valid_campaign_name', 'Invalid campaign name. Only alphanumeric characters, hyphens, underscores and dots are allowed'),
...
export const validatorsCreator = (user, currentSteemDollarPrice, messages, getFieldValue, requiredObject, objectsToAction) => ({
